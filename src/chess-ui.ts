...
export class ChessUI {
  ...
  displayMoveHistory(moves: ChessMove[]): void {
    ...
    for (let i = 0; i < moves.length; i += 2) {
      ...
      if (blackMove) {
        line +=
          chalk.gray("  ") +
          chalk.black(blackMove.san || `${blackMove.from}-${blackMove.to}`);
      }
  ...
  displayMenu(): void {
    ...
    console.log(
      chalk.gray("  move <from>-<to>     ") +
        chalk.white("Make a move (e.g., e2-e4)"),
    );
    console.log(
      chalk.gray("  moves <square>       ") +
        chalk.white("Show legal moves for a square"),
    );
    console.log(
      chalk.gray("  all-moves            ") +
        chalk.white("Show all legal moves"),
    );
    console.log(
      chalk.gray("  history              ") + chalk.white("Show move history"),
    );
    console.log(
      chalk.gray("  status               ") + chalk.white("Show game status"),
    );
    console.log(
      chalk.gray("  board                ") + chalk.white("Display board"),
    );
    console.log(
      chalk.gray("  undo                 ") + chalk.white("Undo last move"),
    );
    console.log(
      chalk.gray("  reset                ") + chalk.white("Reset game"),
    );
    console.log(
      chalk.gray("  fen <position>       ") +
        chalk.white("Load position from FEN"),
    );
    console.log(
      chalk.gray("  pgn <game>           ") + chalk.white("Load game from PGN"),
    );
    console.log(
      chalk.gray("  analyze              ") +
        chalk.white("Analyze current position"),
    );
    console.log(
      chalk.gray("  mode <type> [level]  ") +
        chalk.white("Set game mode (human-vs-human, human-vs-ai, ai-vs-ai)"),
    );
    console.log(
      chalk.gray("  ai                   ") +
        chalk.white("Make AI play a move"),
    );
    console.log(
      chalk.gray("  help                 ") + chalk.white("Show this menu"),
    );
    console.log(
      chalk.gray("  quit                 ") + chalk.white("Exit game"),
    );
  }
  ...
  displayWelcome(): void {
    ...
    console.log(
      chalk.gray("A Model Context Protocol server for playing chess"),
    );
  ...
  displayAnalysis(analysis: any): void {
    ...
    if (analysis.bestMoves && analysis.bestMoves.length > 0) {
      ...
      analysis.bestMoves.forEach((move: ChessMove, index: number) => {
        // Use coordinate format (from-to) instead of algebraic notation
